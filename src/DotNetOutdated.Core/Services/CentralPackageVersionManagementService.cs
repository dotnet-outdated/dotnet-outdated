...
namespace DotNetOutdated.Core.Services
{
    public class CentralPackageVersionManagementService : ICentralPackageVersionManagementService
    {
        ...
        public RunStatus AddPackage(string projectFilePath, string packageName, NuGetVersion version, bool noRestore, TimeSpan timeout)
        {
            ...
            try
            {
                ...
                if (!noRestore)
                {
                    RunStatus restoreStatus = _dotNetRestoreService.Restore(projectFilePath, timeout);
